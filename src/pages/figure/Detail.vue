<template>
  <div class="book-detail">
    ...
    <div class="detail-content" v-if="bookDetailData">
      <div class="detail-linear">
        <header class="detail-top">
          <a @click="back"><i class="back"></i>
            <h2 class="detail-title">{{bookDetailData.name}}</h2>
          </a>
          <router-link to="/" class="home-btn"><i class="iconfont icon-home"></i></router-link>
        </header>
        <div class="detail-con" v-if="!loading">
          <div class="detail-img">
            <img :src="bookDetailData.coverUrl" alt="" :onerror="errorImg">
          </div>
          <div class="detail-main">
            <h3 class="name">{{bookDetailData.name}}</h3>
            <p class="author">作者：{{bookDetailData.authorName}}</p>
            <!--<p class="type">分类：{{bookDetailData.tap}}</p>
            <p class="word-count">{{bookDetailData.wordcount}}万字</p>
            <rate :score="bookDetailData.ratings"></rate>-->
          </div>
        </div>
        <div class="read-btn" v-if="!loading">
          <div>
            <button @click="openBook">
              <router-link :to="{path:'/reader/'+ bookDetailData.code}">开始阅读</router-link>
            </button>
          </div>
          <!--<div>
            <button @click="openBook">
      ...
      <div v-if="!loading">
        <div class="detail-intro">
          <p :class="{show5: !showmore}" @click="showmore = !showmore">{{bookDetailData.description}}</p>
        </div>
        <div class="detail-tag">
          <h3 class="tag">类别标签</h3>
          <ul class="tag-btn clearfix">
            <li v-for="(tItem, tIndex) in bookDetailData.tagDatas" :key=tIndex>{{tItem}}</li>
          </ul>
        </div>
...
import Similar from '../common/Similar.vue'
...
.detail-linear {
  ...
  .detail-top {
    ...
    i.back {
      ...
      width: 30px;
      height: 30px;
      background: url("../../assets/vbook/images/back.png");
    }
    h2 {
      margin: 0 40px;
      font-size: 18px;
